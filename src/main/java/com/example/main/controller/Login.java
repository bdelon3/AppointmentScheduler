...
public class Login {
    ...
    @FXML
    private Label databaseConnectionError;

    @FXML
    private CheckMenuItem contentTheme;
    ...
    public void changeTheme(){
        ...
        if(Styles.getTheme() == "None"){
            while(passwordLabel.getScene().getStylesheets().size() > 0) passwordLabel.getScene().getStylesheets().remove(0);
        }
        else if(Styles.getTheme() == "Dark"){
            while(passwordLabel.getScene().getStylesheets().size() > 0) passwordLabel.getScene().getStylesheets().remove(0);
            passwordLabel.getScene().getStylesheets().add(getClass().getResource("/com/example/main/css/style.css").toExternalForm());
        }
    }
}
